...
MODEL_PATH = os.path.join("..", "..", "models", "20260129-140959.joblib")
...
def predict_endpoint():
    # 1. Récupérer un argument passé dans l’URL
    received_ws1_avg = request.args.get("Ws1_avg", type=float)

    # 2. Construire le dataframe par défaut
    ...
    # 3. Si l’utilisateur a fourni Ws1_avg, on remplace la valeur
    if received_ws1_avg is not None:
        received_data_df["Ws1_avg"] = received_ws1_avg

    # 4. Préparer les features
    prepared = prepare_features(received_data_df, training_mode=False)

    # 5. Faire la prédiction
    prediction_df = predict(prepared, MODEL_PATH)

    # 6. Retourner la prédiction en JSON
    return jsonify({
        "prediction": float(prediction_df["predictions"].iloc[0])
    })
